  ...
  const speak = (text: string, index: number) => {
    ...
    const doSpeak = () => {
      ...
      if (tamilVoice) {
        ...
        // No Tamil voice installed on this device/browser — fall back to default
        // voice so something is still heard, rather than failing silently.
    ...
    if (window.speechSynthesis.getVoices().length === 0) {
      // Voice list not loaded yet — wait for it once, then speak.
