...
const STORAGE_KEY_SESSION = 'kalvi_active_session';
...
export const authService = {
  ...
  login: async (email: string, password: string): Promise<User> => {
    ...
    const mockUsers = JSON.parse(localStorage.getItem('kalvi_mock_users_db') || '[]');
  ...
  getAllUsers: async (): Promise<any[]> => {
    const stored = localStorage.getItem('kalvi_mock_users_db');
  ...
  adminAddUser: async (name: string, email: string, password: string, role: Role): Promise<User> => {
    ...
    localStorage.setItem('kalvi_mock_users_db', JSON.stringify(mockUsers));
  ...
  adminDeleteUser: async (id: string) => {
    ...
    localStorage.setItem('kalvi_mock_users_db', JSON.stringify(updated));
  },
