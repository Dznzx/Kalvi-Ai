...
import { generateChatResponse, synthesizeSpeech } from '../../services/geminiService';
...
export const TamilVoiceAssistant: React.FC<TamilVoiceAssistantProps> = ({ language }) => {
  ...
  // Gemini TTS returns raw 16-bit PCM mono audio at 24kHz, base64-encoded.
  // Browsers can't play raw PCM directly, so we wrap it in a minimal WAV header.
  const pcmBase64ToWavUrl = (base64: string): string => {
  ...
  const speakWithBrowserVoice = (text: string, index: number) => {
    ...
    const doSpeak = () => {
      ...
      if (tamilVoice) {
        ...
        console.warn('No Tamil voice found on this device; using default voice.');
  ...
  const speak = async (text: string, index: number) => {
    ...
    // Primary path: Gemini TTS — generates real Tamil audio server-side,
    // so it works the same on every device regardless of installed OS voices.
    const audioBase64 = await synthesizeSpeech(cleanText);

    if (audioBase64) {
      try {
        const url = pcmBase64ToWavUrl(audioBase64);
        ...
        audio.onerror = () => {
          setSpeakingIndex(null);
          URL.revokeObjectURL(url);
          speakWithBrowserVoice(cleanText, index); // fallback if playback fails
        };
        await audio.play();
        return;
      } catch (e) {
    ...
    // Fallback: browser's built-in voice (only reached if the API call
    // or playback failed — e.g. offline, quota exceeded).
    speakWithBrowserVoice(cleanText, index);
  };
