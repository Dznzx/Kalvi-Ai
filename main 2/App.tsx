...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const savedTheme = localStorage.getItem('bharatStack_theme') as 'light' | 'dark';
  ...
  const handleLoginSuccess = (user: User) => {
    ...
    if (user.role === Role.SUPER_ADMIN) {
        ...
        const savedGrade = localStorage.getItem(`bharatStack_grade_${user.id}`);
        if (savedGrade) {
  ...
  if (currentUser?.role === Role.STUDENT && showGradeSelection) {
      return <StudentOnboarding language={language} onSelect={(g) => { 
          setStudentGrade(g); 
          setShowGradeSelection(false);
          localStorage.setItem(`bharatStack_grade_${currentUser.id}`, g);
      }} />;
  }
        ...
        <div className={theme}>
            ...
            <Navbar 
                ...
                theme={theme} toggleTheme={() => {
                  ...
                  setTheme(next);
                  localStorage.setItem('bharatStack_theme', next);
                }}
            />
