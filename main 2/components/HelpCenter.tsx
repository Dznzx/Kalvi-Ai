  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0B0F19] transition-colors duration-300">
        ...
        <div className="text-center mb-10 md:mb-14">
            <div className="w-14 h-14 md:w-16 md:h-16 bg-orange-100 dark:bg-white/10 text-bharatStack-terracotta rounded-2xl flex items-center justify-center mx-auto mb-6 transform -rotate-6 shadow-sm transition-transform hover:rotate-0">
        ...
        <div className="max-w-xl mx-auto mb-12 md:mb-16 relative group px-2 sm:px-0">
            <Search className="absolute left-5 sm:left-6 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-bharatStack-terracotta transition-colors pointer-events-none" size={20} />
            <input 
                type="text" 
                placeholder={language === 'ta' ? "தேடுக..." : "Search questions..."}
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-12 sm:pl-14 pr-6 py-3.5 md:py-4 rounded-full border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 shadow-sm focus:ring-4 focus:ring-bharatStack-terracotta/10 focus:border-bharatStack-terracotta outline-none transition text-gray-800 dark:text-white placeholder-gray-400 text-sm md:text-base"
            />
        </div>
                            ...
                            <div className="space-y-3 md:space-y-4">
                                {items.map((faq, index) => {
                                    ...
                                    return (
                                        <div 
                                            key={uniqueId} 
                                            className={`bg-white dark:bg-white/5 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] dark:shadow-none border border-gray-100 dark:border-white/10 overflow-hidden hover:shadow-md transition-all duration-300 ${isOpen ? 'ring-1 ring-bharatStack-terracotta/10 border-bharatStack-terracotta/20' : ''}`}
                                        >
                                            <button 
                                            ...
                                            >
                                                ...
                                                <ChevronDown 
                                                    className={`text-bharatStack-terracotta flex-shrink-0 transition-transform duration-300 mt-0.5 md:mt-0 ${isOpen ? 'rotate-180' : ''}`} 
                                                    size={20}
                                                />
                                            </button>
