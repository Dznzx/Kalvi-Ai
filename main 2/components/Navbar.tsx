  ...
  const NavLink = ({ target, label, isActive }: { target: View, label: string, isActive: boolean }) => (
    <button 
        ...
        className={`relative px-4 py-2 text-sm font-bold transition-all duration-300 whitespace-nowrap rounded-lg ${
            isActive 
            ? 'text-bharatStack-terracotta bg-orange-50 dark:bg-orange-500/10' 
            : 'text-gray-600 dark:text-gray-300 hover:text-bharatStack-terracotta hover:bg-gray-50 dark:hover:bg-white/5'
        }`}
    >
        {label}
    </button>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
          ...
          <div className="flex items-center justify-between h-full">
            ...
            <div className="flex items-center gap-8 lg:gap-12">
                ...
                >
                   <BharatStackLogo className="h-10 md:h-12" showText={true} tagline={false} />
                </div>
            ...
            <div className="hidden md:flex items-center gap-4 lg:gap-6">
              ...
              <div className="flex items-center p-1 rounded-full bg-gray-100/50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                  ...
                  <button 
                      ...
                      className="flex items-center gap-1.5 px-3 h-8 rounded-full text-gray-600 dark:text-gray-300 hover:bg-white dark:hover:bg-white/10 hover:shadow-sm hover:text-bharatStack-terracotta transition-all text-xs font-bold uppercase tracking-wider"
                  >
                     <Globe size={14} /> 
              ...
              {!currentUser ? (
                  <div className="flex items-center gap-3">
                      <button 
                          ...
                          className="text-sm font-bold text-gray-600 dark:text-gray-300 hover:text-bharatStack-terracotta transition-colors flex items-center gap-1 px-4 py-2 hover:bg-gray-50 dark:hover:bg-white/5 rounded-full"
                      >
                          {t.login}
                      </button>

                      <button onClick={() => setOnboardingFormOpen(true)} className="bg-bharatStack-terracotta text-white px-5 py-2 rounded-full font-bold text-sm shadow-lg shadow-orange-900/10 hover:bg-[#C2410C] hover:shadow-orange-900/20 transition-all active:scale-95 whitespace-nowrap">
                          {t.register}
                      </button>
                  </div>
              ) : (
                  <div className="flex items-center gap-4">
                    ...
                    {currentUser.avatarUrl ? (
                      ...
                      <div className="w-10 h-10 rounded-full bg-bharatStack-terracotta text-white flex items-center justify-center font-bold text-sm shadow-md flex-shrink-0">
            ...
            <div className="md:hidden flex items-center gap-2">
               ...
               >
                  <Globe size={14} className="text-bharatStack-terracotta" />
      ...
      <div className={`fixed inset-0 z-[60] transition-opacity duration-300 ${mobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 h-full w-[85%] max-w-[300px] bg-white dark:bg-[#1A1F2E] shadow-2xl transition-transform duration-300 ease-out transform ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
          <div className="p-6 border-b border-gray-100 dark:border-white/5 flex justify-between items-center">
            <div className="text-gray-900 dark:text-white"><BharatStackLogo className="h-10" showText={false} /></div>
             ...
             {!currentUser ? (
               <>
                  ...
                  <button onClick={() => { handleLoginClick(); setMobileMenuOpen(false); }} className="w-full text-left font-medium p-3 hover:bg-orange-50 dark:hover:bg-white/5 rounded-xl text-gray-700 dark:text-gray-300 flex justify-between items-center group">
                      {t.login} <LogIn size={16} className="text-gray-300 group-hover:text-bharatStack-terracotta"/>
                  </button>
               </>
             ) : (
               <>
                  <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-xl mb-4 flex items-center gap-3">
                    {currentUser.avatarUrl ? (
                      ...
                      <div className="w-10 h-10 rounded-full bg-bharatStack-terracotta text-white flex items-center justify-center font-bold">
                          {currentUser.name.charAt(0)}
                      </div>
                    )}
          ...
          {!currentUser && (
            <div className="p-6 border-t border-gray-100 dark:border-white/5">
              <button 
                ...
                className="w-full bg-bharatStack-terracotta text-white text-center py-3.5 rounded-xl font-bold shadow-lg shadow-orange-900/20 active:scale-95 transition-all"
              >
                {t.register}
              </button>
            </div>
