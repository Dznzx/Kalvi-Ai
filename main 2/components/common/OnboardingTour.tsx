...
export const OnboardingTour: React.FC<{ language: Language }> = ({ language }) => {
  ...
  const TOUR_KEY = 'bharatStack_student_tour_completed';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-sm w-full relative z-10 pointer-events-auto animate-bounce-in">
        ...
        <div className="mb-4">
            <span className="text-xs font-bold text-bharatStack-blue uppercase tracking-wider">
        ...
        <div className="flex justify-between items-center">
            <div className="flex gap-1">
                {steps.map((_, i) => (
                    <div key={i} className={`w-2 h-2 rounded-full transition ${i === currentStep ? 'bg-bharatStack-blue w-4' : 'bg-gray-200'}`} />
                ))}
            </div>
            <button 
                onClick={handleNext}
                className="bg-bharatStack-blue text-white px-5 py-2 rounded-lg font-bold hover:bg-blue-700 transition flex items-center gap-2"
            >
